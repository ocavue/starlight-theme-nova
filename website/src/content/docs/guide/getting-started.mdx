...
If you want to support multiple languages, you can pass an object with values for each different locale to `label` and `href`. When using the object form, the key must be the name of the corresponding language directory (e.g. `en`, `ar`, or `zh-CN`) or the `root` key to set the default value:
